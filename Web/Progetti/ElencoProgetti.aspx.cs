...
{
    public partial class ElencoProgetti : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void rtbPrincipale_ButtonClick(object sender, RadToolBarEventArgs e)
        {
            try
            {
                if (e != null && e.Item != null)
                {
                    ...
                    if (value == "Esporta")
                    {
                        EffettuaEsportazioneInFileExcel();
                    }
                }
            }
        ...
        /// <summary>
        /// Effettua l'esportazione di tutta la griglia dei progetti in formato excel
        /// </summary>
        private void EffettuaEsportazioneInFileExcel()
        {
            rgGriglia.ExportSettings.FileName = "Elenco_Progetti";
            ...
            rgGriglia.ExportSettings.Excel.Format = GridExcelExportFormat.Xlsx;
            rgGriglia.ExportSettings.Excel.FileExtension = "xlsx";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!Helper.Web.IsPostOrCallBack(this))
                {
                    ...
                    //rgGriglia.MasterTableView.FilterExpression = "(Chiuso = true)";
                    rgGriglia.MasterTableView.FilterExpression = "ChiusoString = \"NO\"";
                    //rgGriglia.MasterTableView.Rebind();
            ...
            switch (e.CommandName)
            {
                case "Clona":
                    try
                    {
                        ...
                        Entities.Progetto newPrj = new Logic.Progetti().Clona(new EntityId<Progetto>(idSelectedRow));
                        Response.Redirect($"/Progetti/DettagliProgetto.aspx?ID={newPrj.ID}");
                    }
                    catch(Exception ex)
                    {
                        MessageHelper.ShowErrorMessage(Page, "Operazione di Clonazione non riuscita, è stato riscontrato il seguente errore:<br />" + ex.Message);
                    }
                    break;
        ...
        protected void rgGriglia_DeleteCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Entities.Progetto entityToDelete = llProgetti.Find(new EntityId<Entities.Progetto>(idSelectedRow));
                    if (entityToDelete != null)
                    {
                        //if (entityToDelete.Progetto_Attivita.Count > 0) throw new Exception($"Per proseguire l'eliminazione è necessario eliminare tutte le attività associato al progetto \"{entityToDelete.Titolo}\" ({entityToDelete.Numero})");

                        llProgetti.Delete(entityToDelete, false);
                        llProgetti.SubmitToDatabase();

                        SeCoGes.Logging.LogManager.AddLogOperazioni(String.Format("{0} - Rimosso l'entity Progetto con ID '{1}'.", Request.Url.AbsolutePath, entityToDelete.ID));
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void rgGriglia_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            ...
            {
                Logic.Progetti llProgetti = new Logic.Progetti();
                IQueryable<Entities.Progetto> elencoOfferte = llProgetti.Read();
                rgGriglia.DataSource = elencoOfferte;
            }
            catch (Exception ex)
            {
                MessageHelper.ShowErrorMessage(this, ex);
