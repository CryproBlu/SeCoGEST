...
{
    public partial class AnalisiCosti : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void rgGriglia_DeleteCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Entities.AnalisiCosto entityToDelete = llAnalisiCosti.Find(new EntityId<Entities.AnalisiCosto>(idSelectedRow));
                    if (entityToDelete != null)
                    {
                        llAnalisiCosti.Delete(entityToDelete, false);
                        llAnalisiCosti.SubmitToDatabase();

                        SeCoGes.Logging.LogManager.AddLogOperazioni(String.Format("{0} - Rimosso l'entity AnalisiCosto con ID '{1}'.", Request.Url.AbsolutePath, entityToDelete.ID));
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void rgGriglia_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            ...
            {
                Logic.AnalisiCosti llAnalisiCosti = new Logic.AnalisiCosti();
                IQueryable<Entities.AnalisiCosto> elencoAnalisiCosti = llAnalisiCosti.Read();
