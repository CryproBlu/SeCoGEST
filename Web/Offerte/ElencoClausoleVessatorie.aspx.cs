...
{
    public partial class ElencoClausoleVessatorie : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void rgGriglia_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                //    Response.Redirect(string.Format("/Offerte/DettagliOfferta.aspx?ID={0}", idValue));
                //    break;
        ...
        protected void rgGriglia_DeleteCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Entities.ClausolaVessatoria entityToDelete = ll.Find(new EntityId<Entities.ClausolaVessatoria>(idSelectedRow));
                    if (entityToDelete != null)
                    {
                        if (entityToDelete.AnalisiVenditaConfigurazioneArticoloAggiuntivoClausolaVessatorias.Count > 0)
                        {
                            throw new Exception(String.Format("Condizione Particolare con ID '{0}', è associata a {1} configurazione articolo aggiuntivo. Per effettuare la rimozione, prima, è necessario rimuovere tali associazioni", entityToDelete.ID, entityToDelete.AnalisiVenditaConfigurazioneArticoloAggiuntivoClausolaVessatorias.Count));
                        }

                        ll.Delete(entityToDelete, false);
                        ll.SubmitToDatabase();

                        SeCoGes.Logging.LogManager.AddLogOperazioni(String.Format("{0} - Rimosso l'entity Condizione Particolare con ID '{1}'.", Request.Url.AbsolutePath, entityToDelete.ID));
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void rgGriglia_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            ...
            {
                Logic.ClausoleVessatorie ll = new Logic.ClausoleVessatorie();
                IQueryable<Entities.ClausolaVessatoria> elencoOfferte = ll.Read();
