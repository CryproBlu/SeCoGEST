...
{
    public partial class ElencoOfferte : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void rgGriglia_DeleteCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Entities.Offerta entityToDelete = llOfferte.Find(new EntityId<Entities.Offerta>(idSelectedRow));
                    if (entityToDelete != null)
                    {
                        llOfferte.Delete(entityToDelete, false);
                        llOfferte.SubmitToDatabase();

                        SeCoGes.Logging.LogManager.AddLogOperazioni(String.Format("{0} - Rimosso l'entity Offerta con ID '{1}'.", Request.Url.AbsolutePath, entityToDelete.ID));
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void rgGriglia_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            ...
            {
                Logic.Offerte llOfferte = new Logic.Offerte();
                IQueryable<Entities.Offerta> elencoOfferte = llOfferte.Read();
