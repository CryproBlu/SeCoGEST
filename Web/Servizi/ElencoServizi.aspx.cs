...
{
    public partial class ElencoServizi : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void rgGriglia_DeleteCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            Logic.Servizi llServizi = new Logic.Servizi();
            ...
            try
            {
                ...
                {
                    Entities.Servizio entityToDelete = llServizi.Find(new EntityId<Entities.Servizio>(idSelectedRow));
                    if (entityToDelete != null)
                    {
                        if (entityToDelete.ServizioArticolos.Count > 0) throw new Exception($"Per proseguire l'eliminazione è necessario eliminare tutti gli articoli associato al servizio \"{entityToDelete.Nome}\" ({entityToDelete.Codice})");

                        llServizi.Delete(entityToDelete, false);
                        llServizi.SubmitToDatabase();

                        SeCoGes.Logging.LogManager.AddLogOperazioni(String.Format("{0} - Rimosso l'entity Servizio con ID '{1}'.", Request.Url.AbsolutePath, entityToDelete.ID));
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void rgGriglia_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            ...
            {
                Logic.Servizi llServizi = new Logic.Servizi();
                IQueryable<Entities.Servizio> elencoOfferte = llServizi.Read();
